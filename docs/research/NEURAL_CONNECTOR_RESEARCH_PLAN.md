...
- [ ] CURRENT TEST PATCH: Nexus V11 refines the existing trunk fibres into shorter local return-forks. Every fine fork must leave and return to a later point on the same trunk, with thinner descendants and fewer internal cross-links; do not leave long uniform ribs hanging in open space.
...
Current drawing ownership: Nexus V11 draws the middle trunks/soma and bounded attached fibres; its one focus root uses a broader blue/violet tube, sustained app-end collar and brighter internal spine. The V10 soma remains one small lobe-shaped body with no cage/web strokes. V11 uses short return-forks and sparse cross-links inside the same existing trunk geometry. Scaffold V7 draws the underlying main/manual connections and local dendrites; Flow V3 owns existing route pulses. Width V18 loads Nexus V11 only. Do not re-enable the retired overlay chain. The V7 trial only altered the outer tissue on the first captured route; its taper made the Memory end appear detached and was removed in V8.

Work from the user's newest screenshot and original neural reference side by side. The current gate is whether the fibres read as fine local anatomy rather than long loose ribs. Pulse expansion is deferred until the design is accepted and is not part of the next root patch.
