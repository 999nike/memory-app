...
- [x] Nexus V10 centre-soma pass accepted on the user's HP screen: the polygon cage is gone and the root bases meet at one small organic junction.
- [x] Nexus V11 fibre pass accepted on the user's HP screen; Settings remains intentionally physics-anchored while expanded, but its connector joins correctly and is not a neural-renderer fault.
- [ ] CURRENT TEST PATCH: Scaffold V8 carries restrained blue/violet tissue and a thin internal spine into app-child links. It limits local dendrites to short compact twigs so child controls retain their green-bubble readability. Do not change central trunks, graph edges, node physics, actions or grouping.
...
Current drawing ownership: Nexus V11 draws the middle trunks/soma and bounded attached fibres; its one focus root uses a broader blue/violet tube, sustained app-end collar and brighter internal spine. The V10 soma remains one small lobe-shaped body with no cage/web strokes. V11 uses short return-forks and sparse cross-links inside the same existing trunk geometry. Scaffold V8 draws restrained tissue/spine child links and compact local dendrites; Flow V3 owns existing route pulses. Width V18 loads Nexus V11 only. Do not re-enable the retired overlay chain. The V7 trial only altered the outer tissue on the first captured route; its taper made the Memory end appear detached and was removed in V8.

Work from the user's newest screenshot and original neural reference side by side. The current gate is whether app-child links read as fine local anatomy rather than electric spokes, while green bubbles remain clear. Pulse expansion is deferred until the design is accepted and is not part of the next root patch.
